...
export default {
    ...
    async getAndInsertRecords(race_url: string, table: string, race_id: number): Promise<void> {
        try {
    ...
    async scrapeAllRacesByGame(game: string, deleteTable: boolean, startingRaceID: number): Promise<void> {
        if (deleteTable) {
    ...
    async getMKWiiUrls(): Promise<string[]> {
    ...
    async getRaceURLs(game: string): Promise<string[]> {
        return new Promise(async (resolve, reject) => {
            if (game === "mkwii") {
                console.log()
                resolve(await this.getMKWiiUrls())
            }
            let race_urls: string[] = []
            let base_url = `https://mkwrs.com/${game}/`
            const html = await fetchHTML(base_url)
            if (!html)
                reject("Error getting HTML.")
            const tables = parse(html).querySelectorAll('table')
