...
const getRaceRecords = async (race_url, game, race_id) => {
    ...
    try {
        ...
        const characterCell = game !== 'mkwii' ? 10 : 8
        const kartCell = game !== 'mkwii' ? 11 : 9
        const durationCell = game !== 'mkwii' ? 4 : 11
        const nationCell = game !== 'mkwii' ? 3 : 4
...
module.exports = {
    ...
    scrapeAllRacesByGame: async function scrapeAllRacesByGame(game, deleteTable, startingRaceID) {
        ...
        for (const url of await this.getRaceURLs(game)) {
    ...
    getMKWiiUrls: async function getMKWiiUrls() {
        return new Promise(async (resolve, reject) => {
            const html = parser.parse(await fetchHTML('https://mkwrs.com/mkwii/'))
            ...
            for (const wrTable of wrTables) {
                ...
                for (const tableRow of tableRows) {
                    ...
                    if (linkCell) {
                        ...
                        if (link && '_rawAttrs' in link && 'href' in link._rawAttrs) {
                            let href = "https://mkwrs.com/mkwii/" + link._rawAttrs.href
                            race_urls.push(href)
                        }
                    }
                }
    ...
    getRaceURLs: async function getRaceURLs(game) {
        return new Promise(async (resolve, reject) => {
            if (game === "mkwii") {
                console.log()
                resolve(await this.getMKWiiUrls())
            }
            race_urls = []
